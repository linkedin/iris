...
### mobile api classes and functions
...
def load_template(name):
    ...
    path = os.path.join('src/iris/ui/templates', name)
...
class TalkToIrisResource(object):
    ...
    def on_get(self, req, resp):
        ...
        if hmac.compare_digest(server_digest,client_digest):
            ...
            template = load_template('mobileIncidents.html')
...
def construct_falcon_api(debug, healthcheck_path, allowed_origins, iris_sender_app, sender_addr):
    ...
    api.add_route('/v0/mobile/push', push)

    api.add_route('/v0/mobile/claim', claim)

    api.add_route('/v0/mobile/getall', getAll)

    api.add_route('/v0/mobile/getactive', getActive)

    api.add_route('/v0/mobile/login', login)#sample to be removed

    api.add_route('/ui/static/images/{filename}', StaticResource('/static/images'))

    #generate qr key
    api.add_route('/v0/mobile/qrkey', qrKey)
    #add device
    api.add_route('/v0/mobile/newdevice', addDevice)
    #connect to iris and generate main page
    api.add_route('/v0/mobile/tti', talkToIris)
