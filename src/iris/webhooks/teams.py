...
class teams(object):
    ...
    def validate_post(self, body):
        if not all(k in body for k in("title", "summary", "text")):
            logger.warning('missing title, summary and/or text attributes')
            raise HTTPBadRequest('missing title, summary and/or text attributes')

    def create_context(self, body):
        context_json_str = ujson.dumps(body)
        if len(context_json_str) > 65535:
            logger.warning('POST to teams exceeded acceptable size')
    ...
    def on_post(self, req, resp):
        '''
        This endpoint is compatible with any webhook post for Microsoft Teams

        Url: http://iris:16649/v0/webhooks/teams?application=test-app&key=sdffdssdf&plan=team1
        '''
