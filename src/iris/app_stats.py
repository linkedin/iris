...
def calculate_app_stats(app, connection, cursor, fields_filter=None):
    queries = {
        ...
        'high_priority_incidents_last_week': '''SELECT application.name, COUNT(DISTINCT incident.id) AS incident_count
                                                FROM incident
                                                JOIN application ON application.id = %(application_id)s
                                                JOIN message ON message.incident_id = incident.id AND application.id = incident.application_id
                                                WHERE incident.created > %(date_variable)s - INTERVAL 1 WEEK
                                                AND incident.created < %(date_variable)s
                                                AND priority_id IN (
                                                    SELECT id FROM priority WHERE name IN ('high', 'urgent')
                                                )
                                                GROUP BY application.id, application.name
...
def calculate_single_stat(connection, cursor, stat_name):
    ...
    queries = {
        ...
        'pct_incidents_claimed_last_week': '''SELECT application.name,
                                                    ROUND(COUNT(owner_id) / COUNT(*) * 100, 2) AS cnt
                                                FROM incident
                                                JOIN application ON application.id = incident.application_id
                                                WHERE created > %(date_variable)s - INTERVAL 1 WEEK
                                                AND created < %(date_variable)s
                                                GROUP BY application.id, application.name
                                                ORDER BY cnt DESC, application.name ASC
                                                ''',
        ...
        'high_priority_incidents_last_week': '''SELECT application.name, COUNT(DISTINCT incident.id) AS incident_count
                                                ...
                                                GROUP BY application.id, application.name
                                                ORDER BY incident_count DESC, application.name ASC
                                                '''
