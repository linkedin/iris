...
def test_create_oncall_team_table(mocker):
    ...
    sample_oncall_users_response = {"abc": {"sms": "+1 999-999-9999", "slack": "abc", "call": "+1 999-999-9999", "email": "abc@domain.com"}, "demo": {"sms": "+1 999-999-9999", "slack": "demo", "call": "+1 999-999-9999", "email": "demo@domain.com"}}
    ...
    iris_team_names = {name for (name, ) in engine.execute('''SELECT `name` FROM `target` WHERE `type_id` = 35''')}
    assert iris_oncall_team_ids == {10001, 10002, 10003}
    assert iris_team_names - {'demo_team', 'foo_team', 'bar_team'} == set({})
...
def test_rename_team(mocker):
    ...
    iris_team_names = {name for (name, ) in engine.execute('''SELECT `name` FROM `target` WHERE `type_id` = 35''')}
    assert iris_oncall_team_ids == {10001, 10002, 10003}
    assert iris_team_names - {'demo_team_renamed', 'foo_team_renamed', 'bar_team'} == set({})
...
def test_delete_team(mocker):
    ...
    # return only bar team. This simulates deleting demo and foo team from oncall
    ...
    assert iris_team_names - {'bar_team'} == set({})
